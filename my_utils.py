...
#   - result_column: Index of the column you want to retrieve new values 
#                    when a match between query_column and query_value is TRUE.
#                    Default is index 1 (the second column in the CSV).
    ...
    with open(file_name, "r") as file:
        for line in file:
            ...
            if values[query_column] == (query_value): #check if the value in the query_column matches the query_value
                results.append(values[result_column]) #If it matches, save the value from the result_column into the results array
